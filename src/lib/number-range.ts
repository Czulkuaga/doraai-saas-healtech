...
export async function peekNextNumberRangeCode(opts: NextNumberOpts): Promise<string> {
    const {
        tenantId,
        object,
        defaultPrefix = null,
    ...
    const current = await db.numberRange.findUnique({
        where: { tenantId_object: { tenantId, object } },
        select: { prefix: true, nextNo: true, padding: true },
    });

    const nextNo = current?.nextNo ?? defaultNextNo;
    const prefix = current?.prefix ?? defaultPrefix;
    const padding = current?.padding ?? defaultPadding;

    const padded = padLeft(nextNo, padding);
