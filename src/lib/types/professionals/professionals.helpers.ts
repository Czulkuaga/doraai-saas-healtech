...
export function getProfessionalDisplayName(input: {
    type: PartnerType;
...
export function getProfessionalInitials(input: {
    type: PartnerType;
    firstName?: string | null;
    lastName?: string | null;
    organizationName?: string | null;
    ...
    if (input.type === PartnerType.ORGANIZATION) {
        const source = input.organizationName?.trim() || input.code?.trim() || "PR";
        return source
            .split(/\s+/)
            .slice(0, 2)
...
export function getProfessionalTypeLabel(type: PartnerType): string {
    switch (type) {
