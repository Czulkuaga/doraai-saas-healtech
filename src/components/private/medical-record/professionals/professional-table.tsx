...
import { deleteProfessionalAction } from "@/action/professionals/delete-professional";
import { toggleProfessionalStatusAction } from "@/action/professionals/toggle-professional";
import {
    getProfessionalDisplayName,
    getProfessionalInitials,
    getProfessionalTypeLabel,
} from "@/lib/types/professionals/professionals.helpers";
import type { ProfessionalListItem } from "@/lib/types/professionals/professionals.types";
...
export function ProfessionalTable({
    ...
    const [selectedItem, setSelectedItem] = useState<ProfessionalListItem | null>(null);
    const [toggleTarget, setToggleTarget] = useState<ProfessionalListItem | null>(null);
    const [toggleNextValue, setToggleNextValue] = useState(false);

    function handleOpenDelete(item: ProfessionalListItem) {
    ...
    function handleOpenToggle(item: ProfessionalListItem) {
    ...
    function handleConfirmDelete() {
        ...
        startTransition(async () => {
            const res = await deleteProfessionalAction(selectedItem.id);
    ...
    function handleConfirmToggle() {
        ...
        startTransition(async () => {
            const res = await toggleProfessionalStatusAction(toggleTarget.id);
    ...
    return (
        <>
            ...
            <ConfirmDialog
                ...
                description={
                    selectedItem
                        ? `Le professionnel ${getProfessionalDisplayName(selectedItem)} sera supprimé définitivement.`
                        : "Ce professionnel sera supprimé définitivement."
                }
            ...
            <ConfirmDialog
                ...
                title={toggleNextValue ? "Réactiver le professionnel" : "Désactiver le professionnel"}
                description={
                    toggleTarget
                        ? toggleNextValue
                            ? `Le professionnel ${getProfessionalDisplayName(toggleTarget)} redeviendra actif dans le module médical.`
                            : `Le professionnel ${getProfessionalDisplayName(toggleTarget)} sera désactivé dans le module médical.`
                        : toggleNextValue
                            ? "Ce professionnel redeviendra actif dans le module médical."
                            : "Ce professionnel sera désactivé dans le module médical."
                }
