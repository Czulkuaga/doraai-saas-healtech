...
import type {
    ProfessionalFilters as QueryType,
    ProfessionalListItem,
} from "@/lib/types/professionals/professionals.types";
import {
    professionalEditPath,
...
export function ProfessionalListShell({
    items,
    totalItems,
    page,
    pageSize,
    ...
    const onQueryChange = (patch: Record<string, string | number | undefined>) => {
        ...
        router.push(`${professionalListPath()}${params.toString() ? `?${params.toString()}` : ""}`);
    };
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                    Gérez les professionnels liés au module de médecine préventive.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="inline-flex items-center rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-sm font-medium text-emerald-700 dark:text-emerald-300">
                            {totalItems} professionnels
                        </div>

                        <button
                            type="button"
                            onClick={() => router.push(professionalNewPath())}
            ...
            <ProfessionalFilters query={query} onChange={onQueryChange} />

            {items.length === 0 ? (
                <ProfessionalEmptyState onCreate={() => router.push(professionalNewPath())} />
            ) : (
                <>
                    <ProfessionalTable
                        items={items}
                        ...
                        onEdit={(id) => router.push(professionalEditPath(id))}
                        onView={(id) => router.push(professionalDetailsPath(id))}
                    />
